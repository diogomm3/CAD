...
export default function App(){
  ...
  const [importUrl,setImportUrl]=useState(''); const [importing,setImporting]=useState(false)
  ...
  async function importModel(e?:React.FormEvent){e?.preventDefault();if(!importUrl.trim()||importing)return;setImporting(true);setError('');setJob({status:'queued',progress:0,message:'Starting URL import…'})
    try{const r=await fetch('/api/models/import',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model_url:importUrl.trim(),query:query.trim()||undefined})});const data=await r.json();if(!r.ok)throw new Error(data.detail||'Could not import this URL');await pollJob(data)}catch(e){setError(e instanceof Error?e.message:'Import failed')}finally{setImporting(false)}}
  ...
  return <main className="shell">
    ...
    <form className="importUrl" onSubmit={importModel}><label htmlFor="model-url">Or import a model directly</label><div><input id="model-url" value={importUrl} onChange={e=>setImportUrl(e.target.value)} placeholder="Paste a public model URL"/><button disabled={importing||!importUrl.trim()}>{importing?<LoaderCircle className="spin" size={15}/>:null}Import</button></div></form>
