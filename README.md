...
A local React and FastAPI app for searching model listings on Printables and MakerWorld, then collecting selected designs under `Projects/` with source attribution and per-file metadata. The application is fully functional when at least one configured source is accessible. Source availability depends on the access method permitted by each website.
...
Each source has an enable flag and access mode: `PRINTABLES_ENABLED` / `PRINTABLES_ACCESS_MODE` and `MAKERWORLD_ENABLED` / `MAKERWORLD_ACCESS_MODE`. Modes are `auto`, `http`, `browser`, `authenticated_browser`, `api`, and `disabled`. `auto` tries normal HTTP, public Playwright, and then the persistent authenticated browser when `BROWSER_AUTH_ENABLED=true`. Unsupported combinations report `unsupported`.
...
```bash
.venv/bin/python -m app.tools.browser_login makerworld
```
...
Run `.venv/bin/python -m app.tools.browser_diagnostic "https://example.com"` to save sanitized HTML, screenshot, console errors, and response URLs under `backend/debug/browser/`. Add `--authenticated` to use the manual-login profile. Set `DEBUG_SCRAPERS=true` to save sanitized fetched HTML/JSON and browser screenshots under `backend/debug/<source>/` when parsing fails. Debug artifacts and the browser profile are ignored by Git. Diagnostic files do not include cookies or request authorization headers; inspect screenshots and page content before sharing them.

### Current live adapter check

The browser smoke test passed: Chromium launched, navigated to `example.com`, and read its title and DOM. Printables search uses its public GraphQL response and returned ten live results for `thor hammer` on 25 September 2026. MakerWorld search uses its public Bambu listing endpoint and returned matching models, images, creator names, licenses, and file metadata for the same query. The providers' direct HTML model pages may still be blocked in this environment, and neither listing response provides public direct download URLs.
...
| MakerWorld | public Bambu listing API succeeds | public file metadata in search response | public direct URL not available | search and file discovery |

The latest end-to-end command attempt in this environment received HTTP 403 from Printables at model-detail retrieval after GraphQL search succeeded. No source is marked verified until search, model detail retrieval, file discovery, and a validated download all pass.
...
This is a local MVP. Public site HTML is not a stable API; result titles, authors, popularity statistics, licenses, and files may be missing. Some model pages need a normal logged-in user session or site-specific download actions. Downloads can therefore be partial. Job progress is held in memory and resets if the backend restarts. The API response shows current per-source state, and the UI offers a retry for each failed source.
